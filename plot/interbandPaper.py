...
def asOfFreq():
    ...
    for wDInd, wD in enumerate(wDArr):
        ...
        filename = "./../data/gsProp2BandsUa0Ub0Uud0Uss0epsA0epsB100gE1WD{}FD500NB2TS20WPh{}.hdf5".format(int(np.rint(wD * 100)), int(np.rint(wD * 1000)))
        #print(filename)
        file = h5py.File(filename,'r')
        #times = file['times'][()]
        #pump = file['pump'][()]
    ...
    gs = gridspec.GridSpec(1, 1, hspace=0., top=.93, bottom=0.05, left=0.2, right=0.96)
    ...
    ax.plot(wDArr, dOccIntraMean, color="teal", linewidth=1., label=r"$\mathrm{Driven}$")
    ax.plot(wPhArr, dOccIntra, color="peru", linewidth=1., label=r"$\mathrm{GS}$")
    ...
    ax.text(-0.35, 1.07, r"$\rm{(a)}$", transform = ax.transAxes, fontsize = 10)
...
def plotTimeEvolCorrSideBySide():
    ...
    for wInd, wVal in enumerate(wArr):
        ...
        filename = "./../data/gsProp2BandsUa0Ub0Uud0Uss0epsA0epsB100gE1WD{}FD500NB2TS20WPh{}.hdf5".format(int(np.rint(wVal * 100)), int(np.rint(wVal * 1000)))
        #print(filename)
        file = h5py.File(filename,'r')
        times = file['times'][()]
        pump = file['pump'][()]
    ...
    nrow = 2
    ncol = 2
    fig = plt.figure(figsize=(3.5, 1.5), dpi=800)

    gs = gridspec.GridSpec(nrow, ncol, width_ratios=[1, 1], height_ratios=[5, 2],
                           hspace=0.1, wspace=0.05, top=0.95, bottom=0., left=0.2, right=0.98)
    ...
    ax1Drive.axvline(tArr[0,300] * wArr[0] / 2. / np.pi, color = 'black', lw = 0.5)
    ax1Drive.axvline(tArr[0,1300] * wArr[0] / 2. / np.pi, color = 'black', lw = 0.5)

    ax2Drive.axvline(tArr[0,300] * wArr[0] / 2. / np.pi, color = 'black', lw = 0.5)
    ax2Drive.axvline(tArr[0,1300] * wArr[0] / 2. / np.pi, color = 'black', lw = 0.5)
    ...
    ax1.set_ylabel(r"$C$", fontsize = 8)
    ax1Drive.set_ylabel(r"$F(t) \sin(\omega_{\rm D}t)$", labelpad=8, fontsize = 8)
    ax2NBos.set_ylabel(r"$N_{\rm bos}$", fontsize = 8)
    ...
    ax1.set_xticks([])
    ax2.set_xticks([])
    ax1Drive.set_xlabel(r"$t[\frac{2 \pi}{\omega_{\rm D}}]$", fontsize = 8)
    ax2Drive.set_xlabel(r"$t[\frac{2 \pi}{\omega_{\rm D}}]$", fontsize = 8)

    ax1Drive.set_xticks([0, 15, 65, 100, 200])
    ax1Drive.set_xticklabels([r"$0$", r"$t_0$", r"$t_1$", r"$100$", r"$200$"], fontsize = 8)

    ax2Drive.set_xticks([0, 15, 65, 100, 200])
    ax2Drive.set_xticklabels([r"$0$", r"$t_0$", r"$t_1$", r"$100$", r"$200$"], fontsize = 8)

    ax2.set_yticks([])
    ax1NBos.set_yticks([])
    ax2Drive.set_yticks([])

    legend = ax1.legend(fontsize=8, loc='upper left', bbox_to_anchor=(-.07, 1.3), edgecolor='black', ncol=2)
    legend.get_frame().set_alpha(0.)
    legend.get_frame().set_boxstyle('Square', pad=0.1)
    legend.get_frame().set_linewidth(0.0)

    legend = ax2NBos.legend(fontsize=8, loc='upper right', bbox_to_anchor=(1.05, 1.25), edgecolor='black', ncol=1)
    legend.get_frame().set_alpha(0.)
    legend.get_frame().set_boxstyle('Square', pad=0.1)
    legend.get_frame().set_linewidth(0.0)

    legend = ax1Drive.legend(fontsize=7, loc='upper right', bbox_to_anchor=(1., 1.1), edgecolor='black', ncol=1)
    legend.get_frame().set_alpha(0.)
    legend.get_frame().set_boxstyle('Square', pad=0.1)
    legend.get_frame().set_linewidth(0.0)

    ax1.text(-0.35, 1.1, r"$\rm{(b)}$", transform = ax1.transAxes, fontsize = 10)
    ax1.text(0.3, 0.8, r"$\omega_{\mathrm{D}} = 0.9\Delta E$", transform = ax1.transAxes, fontsize = 8)
    ax2.text(0.3, 0.8, r"$\omega_{\mathrm{D}} = \Delta E$", transform = ax2.transAxes, fontsize = 8)
    ...
    plt.savefig('./savedPlots/timeEvolCorrSideNew.png', format='png', bbox_inches='tight', dpi=600)
